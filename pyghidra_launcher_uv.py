...
# TODO: Fix from venv command to uv command
    ...
    #         pass
    result = subprocess.run(['uv', 'run', 'python', '-c', 'import sys; print("{0}.{1}".format(*sys.version_info))'], capture_output=True, text=True)
    version = result.stdout.strip()
    if result.returncode == 0 and version in supported:
...
def get_python_path(python_cmd: List[str]) -> str:
    """Get the full path to the Python executable."""
    result = subprocess.run(python_cmd + ['-c', 'import sys; print(sys.executable)'], capture_output=True, text=True)
...
def create_ghidra_venv(python_cmd: List[str], venv_dir: Path) -> None:
    ...
    subprocess.run(['uv', 'venv', str(venv_dir.absolute())])
...
def get_package_version(python_cmd: List[str], package: str) -> str:
    ...
    result = subprocess.run(['uv', 'pip', 'show', '--python', python_path, package], capture_output=True, text=True)
...
def install(install_dir: Path, python_cmd: List[str], uv_pip_args: List[str], offer_venv: bool) -> List[str]:
    ...
    if install_choice.lower() in ('y', 'yes'):
        if offer_venv:
            ...
            if ghidra_venv_choice.lower() in ('y', 'yes'):
                ...
                return None 
        subprocess.check_call(['uv'] + uv_pip_args)
        return python_cmd
    elif not install_choice.lower() in ('n', 'no'):
        print('Please answer yes or no.')
    return None     

def upgrade(python_cmd: List[str], uv_pip_args: List[str], dist_dir: Path, current_pyghidra_version: str) -> bool:
...
def main() -> None:
    ...
    if args.dev:
        ...
        python_path = get_python_path(python_cmd)
        uv_pip_args: List[str] = ['pip', 'install', '--python', python_path, '--no-index', '-f', str(dist_dir), 'pyghidra']
        ...
        if current_pyghidra_version is None:
            python_cmd = install(install_dir, python_cmd, uv_pip_args, offer_venv)
            if not python_cmd:
                sys.exit(1)
        else:
            upgrade(python_cmd, uv_pip_args, dist_dir, current_pyghidra_version)
